...
module.exports = {
	...
	async execute(message, args) {
        ...
        try {
            ...
            await appendFile(filePath, `${os.EOL}${args[1]}`);
            ...
            message.channel.send(
                ...
                .setTimestamp()
            ).then(message => message.delete({ timeout: 5000 }).catch(() => {})); // Automatically delete the message after 5 seconds
        } catch (error) {
            log.error('Add command error:', error);
            message.channel.send(
                new MessageEmbed()
                .setColor(config.color.red)
                .setTitle('Error!')
                .setDescription(`Failed to add account: ${error.message}`)
                .setFooter(message.author.tag, message.author.displayAvatarURL())
                .setTimestamp()
            ).then(message => message.delete({ timeout: 5000 }).catch(() => {}));
        }
    }
};
